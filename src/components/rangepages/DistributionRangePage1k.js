  ...
  useEffect(() => {
    const fetchWallets = async () => {
      try {
        ...
        // Fetch wallets within the range 1K to 10K
        const walletsData = await getWalletsInRange(1000, 10000);
        ...
        <div className="block-overview mb-4">
          <div className="d-flex flex-row w-100">
            <h4 className="block-overview-header text-center w-100 mt-4">
              <FaWallet className={loading ? "rotate" : ""} size="1.7rem" /> 1K
              to 10K KLS
            </h4>
          </div>
...
export default DistributionRangePage1k;
