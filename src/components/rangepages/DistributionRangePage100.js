...
const DistributionRangePage100 = () => {
  ...
  useEffect(() => {
    const fetchWallets = async () => {
      try {
        ...
        // Fetch wallets within the range 100 to 1K
        const walletsData = await getWalletsInRange(100, 1000);
        ...
        <div className="block-overview mb-4">
          <div className="d-flex flex-row w-100">
            <h4 className="block-overview-header text-center w-100 mt-4">
              <FaWallet className={loading ? "rotate" : ""} size="1.7rem" /> 100
              to 1K KLS
            </h4>
          </div>
...
export default DistributionRangePage100;
